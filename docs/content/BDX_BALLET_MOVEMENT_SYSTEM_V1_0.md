...
| 項目 | 内容 |
|---|---|
| 文書種別 | BDXプロダクト設計資料／動作体系 |
| バージョン | v1.0 |
| 作成日 | 2026-09-02 |
| 対象 | MOVEコンテンツ制作、Personalization、AI推薦、Progression設計 |
| 文書ステータス | 専門監修前ドラフト |
| 専門監修 | 鈴ちゃんによる確認を前提とする |
...
1. [表紙](#1-表紙)
2. [BDXにおけるバレエの考え方](#2-bdxにおけるバレエの考え方)
3. [なぜ動作体系が必要なのか](#3-なぜ動作体系が必要なのか)
...
12. [Progression Tree](#12-progression-tree)
13. [BDX基本動作18](#13-bdx基本動作18)
14. [BDX基本動作・発展動作・バレエスキルの分類](#14-bdx基本動作発展動作バレエスキルの分類)
15. [初心者向けスケーリング](#15-初心者向けスケーリング)
16. [Movement Ontology](#16-movement-ontology)
17. [Personalizationとの接続](#17-personalizationとの接続)
18. [AI推薦例](#18-ai推薦例)
...
**BDX バレエ動作体系 v1.0**は、バレエ動作を難しい技の一覧としてではなく、一般ユーザーからバレエ経験者までが段階的に身体を使うための共通基盤として整理する。

本資料の目的は、「バレエにはどのような動作があるか」を説明することではない。MOVEコンテンツが100本、200本と増えても、すべてを同じ体系の中に位置づけ、ユーザーの状態、能力、経験に応じて適切な動作を選択できる状態をつくることである。
...
動作体系がないままMOVEコンテンツを増やすと、難易度の基準、前提動作、初心者向け変形、動作間のつながりがコンテンツごとに分断される。BDXバレエ動作体系は、コンテンツとPersonalization Engineの間に置く共通言語である。

| 課題 | 動作体系が提供するもの |
|---|---|
| コンテンツごとに難易度の意味が異なる | 7段階と3分類による共通基準 |
| 目的から動作を直接選びやすい | 身体要求、前提、Safetyを介した選択 |
| 初心者用と経験者用が別物になる | 同じ動作のスケーリング |
| 次に何へ進むか説明できない | Progression Treeと前提動作 |
| AI推薦の理由を説明しにくい | 構造化された動作属性 |
| MOVEが増えるほど管理が難しくなる | Movement Ontologyへ移行可能な項目設計 |

```mermaid
flowchart LR
    U[BDXユーザー] --> C[Today's Condition]
    C --> P[Personalization]
    P --> S[バレエ動作体系]
    S --> M[MOVEコンテンツ]
    M --> A[実践]
...
| レベル | 正式名称 | 身につけること | 役割 |
|---:|---|---|---|
| 1 | 基礎動作 | 姿勢、支持、脚と腕の基本操作 | すべての土台 |
| 2 | 可動性とコントロール | 動脚と支持側を分けて扱う | 動きながら軸を保つ |
| 3 | バランスとライン | 片脚支持、姿勢保持、全身協調 | 身体ラインをつくる |
| 4 | 移動と重心移動 | 空間の中で身体を運ぶ | エクササイズから踊る動きへの橋渡し |
| 5 | ジャンプ | 床を押す、空中へ移動する、着地する | 動的負荷を扱う |
| 6 | 回転 | 軸を保ちながら方向を変える | 片脚支持と方向転換を統合する |
| 7 | 複合動作 | 複数動作を連続させる | 動作を「文章」にする |
...
以下の各レベル表では、動作概要を「通常動作」として扱う。身体要求は、コンテンツ設計用の初期値であり、医学的評価ではない。

- 身体要求の順序：**下半身・上半身・体幹・股関節・膝関節・足関節・バランス・可動性・筋力・協調性**
- 値：低／中／高
- 動作特性：その場／移動、両脚／片脚／混合、ジャンプ、回転、衝撃、速度、複雑性
...
| 動作名 | 分類 | 動作概要 | 身体要求：下・上・体・股・膝・足・バ・可・筋・協 | 動作特性 |
|---|:---:|---|---|---|
| デミ・プリエ | A | 両脚で支持し、姿勢を保ちながら膝と股関節を曲げ伸ばす | 中・低・中・中・中・中・低・中・中・中 | その場／両脚／跳なし／回なし／衝撃低／低速／複雑性低 |
| グラン・プリエ | B | より大きな可動域で沈み、立位へ戻る | 高・低・高・高・高・高・中・高・高・中 | その場／両脚／跳なし／回なし／衝撃低／低速／複雑性中 |
| バットマン・タンデュ | A | 支持脚を保ち、足先を床から離さず一方向へ伸ばして戻す | 中・低・中・中・低・中・中・中・中・中 | その場／片脚中心／跳なし／回なし／衝撃低／低〜中速／複雑性低 |
| デガジェ | A | バットマン・タンデュから足先を床面より離して素早く出し戻す | 中・低・中・中・低・中・中・中・中・中 | その場／片脚中心／跳なし／回なし／衝撃低／中速／複雑性中 |
| ルルヴェ | A | 足裏全体の支持から踵を上げ、軸を保って戻す | 中・低・中・低・低・高・中・中・中・中 | その場／両脚または片脚／跳なし／回なし／衝撃低／低〜中速／複雑性中 |
| ポール・ド・ブラ | A | 姿勢と呼吸を保ちながら腕を滑らかに運ぶ | 低・中・中・低・低・低・低・中・低・中 | その場／両脚／跳なし／回なし／衝撃低／低速／複雑性低 |
| カンブレ | A | 軸を失わない範囲で上体を前後または側方へ動かす | 低・中・高・中・低・低・中・高・中・中 | その場／両脚／跳なし／回なし／衝撃低／低速／複雑性中 |
| エポールマン | B | 肩、頭、視線と身体の方向を協調させる | 低・中・中・低・低・低・中・中・低・高 | その場／両脚または片脚／跳なし／回なし／衝撃低／低速／複雑性中 |
...
| 動作名 | 分類 | 動作概要 | 身体要求：下・上・体・股・膝・足・バ・可・筋・協 | 動作特性 |
|---|:---:|---|---|---|
| ロン・ド・ジャンブ・ア・テール | A | 足先で床上に弧を描き、股関節から脚の方向を変える | 中・低・中・高・低・中・中・高・中・中 | その場／片脚中心／跳なし／回なし／衝撃低／低速／複雑性中 |
| フォンデュ | A | 支持脚と動脚を協調させて曲げ、伸ばす | 高・低・高・高・中・中・高・中・高・高 | その場／片脚中心／跳なし／回なし／衝撃低／低速／複雑性中 |
| フラッペ | B | 動脚を素早く伸ばし、明確に出し戻す | 中・低・中・中・低・中・中・中・中・高 | その場／片脚中心／跳なし／回なし／衝撃低／高速／複雑性中 |
| プティ・バットマン | B | 膝の位置を保ち、下腿を細かく切り替える | 中・低・高・中・中・中・高・中・中・高 | その場／片脚中心／跳なし／回なし／衝撃低／高速／複雑性高 |
| パッセ | A | 片脚を支持脚の近くに通過させて運ぶ | 中・低・中・中・中・低・中・中・中・中 | その場／片脚中心／跳なし／回なし／衝撃低／低〜中速／複雑性中 |
| ルティレ | A | 片脚を支持脚の膝付近に保持する | 中・低・高・高・中・中・高・中・中・高 | その場／片脚／跳なし／回なし／衝撃低／低速／複雑性中 |
| デヴェロッペ | A | 曲げた動脚をコントロールしながら一方向へ伸ばす | 高・低・高・高・中・中・高・高・高・高 | その場／片脚／跳なし／回なし／衝撃低／低速／複雑性高 |
| グラン・バットマン | A | 動脚を大きく振り上げ、制御して戻す | 高・低・高・高・中・中・高・高・高・高 | その場／片脚中心／跳なし／回なし／衝撃中／中〜高速／複雑性高 |
| ロン・ド・ジャンブ・アン・レール | B | 動脚を空中に保ち、下腿または脚で弧を描く | 高・低・高・高・中・中・高・高・高・高 | その場／片脚／跳なし／回なし／衝撃低／低〜中速／複雑性高 |
| ロン・ド・ジャンブ・ジュテ | B | 脚を投げ出すような動きと弧の方向変化を組み合わせる | 高・低・高・高・中・中・高・高・高・高 | その場／片脚中心／跳なし／回なし／衝撃中／中〜高速／複雑性高 |
...
| 動作名 | 分類 | 動作概要 | 身体要求：下・上・体・股・膝・足・バ・可・筋・協 | 動作特性 |
|---|:---:|---|---|---|
| アラベスク | A | 片脚支持で反対脚を後方へ伸ばし、全身のラインを保つ | 高・中・高・高・中・中・高・高・高・高 | その場／片脚／跳なし／回なし／衝撃低／低速／複雑性高 |
| アティテュード | B | 片脚支持で動脚を曲げた形に保ち、姿勢を維持する | 高・中・高・高・中・中・高・高・高・高 | その場／片脚／跳なし／回なし／衝撃低／低速／複雑性高 |
| アラベスク・パンシェ | C | アラベスクから上体と動脚のラインを傾ける | 高・中・高・高・中・中・高・高・高・高 | その場／片脚／跳なし／回なし／衝撃低／低速／複雑性高 |
| ルルヴェ・パッセ | B | ルルヴェ上で片脚をパッセまたはルティレの位置に保つ | 高・低・高・高・中・高・高・中・高・高 | その場／片脚／跳なし／回なし／衝撃低／低速／複雑性高 |
| ルルヴェ・アラベスク | B | ルルヴェ上でアラベスクのラインを保つ | 高・中・高・高・中・高・高・高・高・高 | その場／片脚／跳なし／回なし／衝撃低／低速／複雑性高 |
| デヴェロッペ・ア・ラ・スゴンド | B | デヴェロッペで動脚を側方へ伸ばし保持する | 高・中・高・高・中・中・高・高・高・高 | その場／片脚／跳なし／回なし／衝撃低／低速／複雑性高 |
...
| 動作名 | 分類 | 動作概要 | 身体要求：下・上・体・股・膝・足・バ・可・筋・協 | 動作特性 |
|---|:---:|---|---|---|
| タン・リエ | A | 脚を伸ばす動作と体重移動をつなぎ、支持脚を乗り換える | 中・低・中・中・中・中・中・中・中・高 | 移動／混合支持／跳なし／回なし／衝撃低／低速／複雑性中 |
| トンベ | A | 一方向へ重心を落とし、新しい支持脚へ移る | 中・低・中・中・中・中・中・中・中・高 | 移動／片脚から片脚／跳なし／回なし／衝撃低〜中／中速／複雑性中 |
| パ・ド・ブレ | A | 小さなステップを連続し、重心と方向を移す | 中・低・中・中・中・中・中・中・中・高 | 移動／支持切替／跳なし／回なし／衝撃低／中速／複雑性中 |
| シャッセ | A | 一方の脚が他方を追うように移動する | 中・低・中・中・中・中・中・中・中・高 | 移動／支持切替／小さな浮きあり得る／回なし／衝撃低〜中／中速／複雑性中 |
| バランセ | A | 左右または前後へ揺れるように重心を移す | 中・中・中・中・中・中・中・中・中・高 | 移動／支持切替／跳なし／回なし／衝撃低／中速／複雑性中 |
| グリッサード | B | 床を滑るように脚を開閉して移動する | 高・低・高・中・中・中・高・中・高・高 | 移動／支持切替／小ジャンプ／回なし／衝撃中／中速／複雑性高 |
| ピケ | B | 伸ばした脚へ踏み込み、明確に重心を乗せる | 中・低・高・中・中・中・高・中・中・高 | 移動／片脚／跳なし／回なし／衝撃低〜中／中速／複雑性中 |
| パ・ド・バスク | B | 方向と支持脚を変えながら弧を描くように移動する | 高・中・高・高・中・中・高・中・高・高 | 移動／支持切替／跳なしまたは小さな浮き／方向転換／衝撃中／中速／複雑性高 |
...
| 動作名 | 分類 | 動作概要 | 身体要求：下・上・体・股・膝・足・バ・可・筋・協 | 動作特性 |
|---|:---:|---|---|---|
| ソテ | B | 両脚または片脚で床を押し、同じ支持構造へ着地する | 高・低・高・中・高・高・中・中・高・高 | その場／両脚または片脚／跳あり／回なし／衝撃中／中速／複雑性中 |
| シャンジュマン | B | 跳躍中に脚の前後関係を入れ替えて着地する | 高・低・高・中・高・高・高・中・高・高 | その場／両脚／跳あり／回なし／衝撃中／高速／複雑性高 |
| エシャッペ・ソテ | B | 閉じた脚位から跳び、開いた脚位へ移って戻る | 高・低・高・高・高・高・高・中・高・高 | その場／両脚／跳あり／回なし／衝撃中／中〜高速／複雑性高 |
| アッサンブレ | B | 一方の脚から跳び、空中で脚を集めて両脚で着地する | 高・低・高・高・高・高・高・中・高・高 | 移動／片脚から両脚／跳あり／回なし／衝撃中〜高／高速／複雑性高 |
| ジュテ | B | 一方の脚から他方の脚へ跳び移る | 高・低・高・高・高・高・高・中・高・高 | 移動／片脚から片脚／跳あり／回なし／衝撃中〜高／高速／複雑性高 |
| シソンヌ | B | 両脚から跳び、主に片脚へ着地する | 高・低・高・高・高・高・高・高・高・高 | 移動／両脚から片脚／跳あり／回なし／衝撃高／高速／複雑性高 |
| パ・ド・シャ | C | 脚を順に引き上げながら空中を移動する | 高・中・高・高・高・高・高・高・高・高 | 移動／片脚から片脚／跳あり／回なし／衝撃高／高速／複雑性高 |
| アントルシャ・カトル | C | 空中で脚を素早く交差させ、両脚で着地する | 高・低・高・中・高・高・高・中・高・高 | その場／両脚／跳あり／回なし／衝撃高／高速／複雑性高 |
| ブリゼ | C | 移動する跳躍の中で脚を打ち合わせる | 高・中・高・高・高・高・高・高・高・高 | 移動／片脚から片脚／跳あり／方向変化あり／衝撃高／高速／複雑性高 |
| グラン・ジュテ | C | 大きく空間を移動し、一方の脚から他方へ跳ぶ | 高・中・高・高・高・高・高・高・高・高 | 移動／片脚から片脚／跳あり／回なし／衝撃高／高速／複雑性高 |
...
| 動作名 | 分類 | 動作概要 | 身体要求：下・上・体・股・膝・足・バ・可・筋・協 | 動作特性 |
|---|:---:|---|---|---|
| ストゥニュー・ターン | B | 両脚をまとめる動きと方向転換を連続させる | 中・中・高・中・中・中・高・中・中・高 | 移動／両脚中心／跳なし／回あり／衝撃低／中速／複雑性高 |
| シェネ | B | 小さなステップを連続しながら回転して移動する | 高・中・高・中・中・中・高・中・高・高 | 移動／支持切替／跳なし／連続回転／衝撃低〜中／中〜高速／複雑性高 |
| ピケ・ターン | B | 伸ばした脚へ踏み込み、片脚支持で回転する | 高・中・高・中・中・高・高・中・高・高 | 移動／片脚／跳なし／回あり／衝撃低〜中／中速／複雑性高 |
| ピルエット・アン・ドゥオール | B | 片脚支持で身体を外方向へ回転させる | 高・中・高・高・中・高・高・中・高・高 | その場／片脚／跳なし／回あり／衝撃低／中速／複雑性高 |
| ピルエット・アン・ドゥダン | B | 片脚支持で身体を内方向へ回転させる | 高・中・高・高・中・高・高・中・高・高 | その場／片脚／跳なし／回あり／衝撃低／中速／複雑性高 |
| アティテュード・ターン | C | アティテュードの形を保ちながら片脚で回転する | 高・中・高・高・中・高・高・高・高・高 | その場または移動／片脚／跳なし／回あり／衝撃低／中速／複雑性高 |
| フェッテ | C | 動脚の運動と支持脚の回転を連続させる | 高・高・高・高・高・高・高・高・高・高 | その場／片脚／跳なし／連続回転／衝撃中／高速／複雑性高 |
...
| 動作名 | 分類 | 動作概要 | 身体要求：下・上・体・股・膝・足・バ・可・筋・協 | 動作特性 |
|---|:---:|---|---|---|
| アダージオ | B | ゆっくりしたテンポで、姿勢、バランス、脚のコントロールを連結する | 高・高・高・高・中・中・高・高・高・高 | その場または移動／混合支持／跳なし／回あり得る／衝撃低／低速／複雑性高 |
| プティ・アレグロ | B | 小さく速いジャンプやステップを連続させる | 高・中・高・中・高・高・高・中・高・高 | 移動／支持切替／跳あり／回あり得る／衝撃中〜高／高速／複雑性高 |
| グラン・アレグロ | C | 大きな移動と跳躍を組み合わせる | 高・高・高・高・高・高・高・高・高・高 | 大きく移動／支持切替／跳あり／回あり得る／衝撃高／高速／複雑性高 |
| アンシェヌマン | B | 目的に応じて複数の動作を一つの流れへ組み立てる | 中〜高・中〜高・高・中〜高・中〜高・中〜高・高・中〜高・中〜高・高 | その場または移動／混合支持／構成による／構成による／構成による／可変／複雑性高 |
| マネージュ | C | 円形の軌道などを使い、移動、跳躍、回転を連続させる | 高・高・高・高・高・高・高・高・高・高 | 大きく移動／支持切替／跳あり得る／回あり／衝撃中〜高／高速／複雑性高 |
...
| 身体的要求 | 下半身、上半身、体幹、股関節、膝関節、足関節、バランス、可動性、筋力、協調性 | 必要能力との照合 |
| 動作特性 | その場／移動、両脚／片脚、ジャンプ、回転、衝撃、速度、複雑性 | 環境・負荷・難易度の判定 |
...
| 確認項目 | 判定 | 根拠 |
|---|:---:|---|
| 7段階すべて掲載 | PASS | セクション4〜11に掲載 |
| 7段階タイトルが日本語 | PASS | 基礎動作〜複合動作を日本語で統一 |
| バレエ動作名がカタカナ | PASS | 54項目と図内の動作名をカタカナで統一 |
| 基本動作18を掲載 | PASS | セクション13に18項目を掲載 |
| Progression Tree掲載 | PASS | セクション12に4系統と複合接続を掲載 |
| 3分類掲載 | PASS | セクション14にA・B・C、合計54項目を掲載 |
| 初心者スケーリング掲載 | PASS | セクション15に6つの調整軸を掲載 |
| Movement Ontology説明 | PASS | セクション16に項目、構造例、設計原則を掲載 |
| Personalization接続説明 | PASS | セクション17に入力、判断、除外条件を掲載 |
| Safetyへの配慮 | PASS | 全動作表とジャンプ・回転・スケーリングに掲載 |
| 流派差への注意書き | PASS | 冒頭の重要な前提に掲載 |
| 専門監修対象明示 | PASS | 全動作の監修欄とセクション19・付録Aに掲載 |
